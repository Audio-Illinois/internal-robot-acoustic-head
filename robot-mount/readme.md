...
| Small-size Stepper Motor                                 | 1        |  None                            | [digikey](https://www.digikey.com/en/products/detail/adafruit-industries-llc/324/5022791)|
| | | |
...
Run 
```bash
python3 ./teleop.py $PATH
```

Example commands are `home`, which can be used to check that the homing induction sensor is wired properly, and `move $DEG $SPEED` which is used to check that the motor calibration is correct. 
